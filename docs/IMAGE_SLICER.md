...
Util-Tools의 **이미지 슬라이서 스튜디오(Image Slicer Studio)**는 웹툰, 긴 웹페이지 캡처, 디자인 시안, 카드 뉴스 등의 대형 이미지를 **자유 경계선**, **다중 절단선**, **균등 N등분**, **고정 픽셀 간격**, **스마트 여백 자동 감지** 방식으로 정밀하게 분할하고 ZIP 압축 또는 폴더로 일괄 내보내는 고성능 그래픽 유틸리티입니다.
...
flowchart TD
    ...
    subgraph PillowBackend ["3. Python Pillow 고속 절단 엔진 (services/image_slicer_service.py)"]
...
3. **📏 고정 픽셀(Fixed Pixel) 일괄 생성**:
   ...
   - 쇼핑몰 상세페이지나 웹툰 컷 분할 시 극대화된 생산성 제공.
4. **🔢 균등 N등분(Equal Parts) 분할**:
   - 3등분, 4등분, 9등분 등 지정한 개수로 오차 없이 정확히 등분 분할.
...
- **`Ctrl + V` 즉시 붙여넣기**: 캡처 도구(알캡처, 윈도우 캡처 `Win + Shift + S`)로 캡처한 이미지를 파일로 저장할 필요 없이 슬라이서 화면에서 `Ctrl + V`만 누르면 캔버스에 즉시 로드됩니다.
- **줌 & 팬 뷰포트**: 20,000px 이상의 초고해상도 이미지도 마우스 휠 줌(Zoom 10% ~ 500%)과 스페이스바/우클릭 드래그 패닝(Pan)으로 부드럽게 탐색 가능.
