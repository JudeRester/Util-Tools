...
- 대용량(100MB+) 백업 파일을 브라우저 자바스크립트의 `FileReader`와 DOM `<textarea>`로 읽어올 경우, V8 힙 메모리가 800MB 이상 치솟아 브라우저 탭이 강제 종료(OOM 크래시)되거나 수 분간 프리징되는 치명적인 문제가 발생했습니다.
...
sequenceDiagram
    ...
    Note over Back: 브라우저 메모리 소모 0MB!<br>Python Tkinter가 파일 경로 직접 획득
    ...
    Note over Back,DB: 단일 트랜잭션 (with conn:)<br>executemany 일괄 고속 주입
...
## 2. 🌟 듀얼 백업 포맷 (JSON & 90% 압축 ZIP)

1. **단일 JSON 포맷 (`.json`)**:
   - 버전 명세, 내보낸 시각, 10개 모듈의 전체 테이블 레코드를 인간이 읽을 수 있는 표준 JSON 형태로 저장.
   - 타 기기 마이그레이션 및 버전 독립적 데이터 복원 지원.
2. **초고압축 ZIP 아카이브 (`.zip`)**:
   - `backup_manifest.json` 메타데이터와 대용량 텍스트를 `zipfile.ZIP_DEFLATED` 최고 압축률로 패킹.
   - **실측치**: 139.3MB 원본 JSON 데이터를 **13.8MB (약 90.1% 압축)**로 압축하여 디스크 공간 절약 및 고속 전송 가능.
...
- **🔀 병합 복원 (Merge Mode)**:
  ...
  - 동일한 `id`를 가진 레코드가 존재할 경우 최신 업데이트 일시(`updated_at`)를 기준으로 충돌을 자동 해결하거나 `INSERT OR REPLACE` 처리.
- **⚡ 완전 교체 (Replace Mode)**:
  - 지정한 모듈의 기존 테이블 데이터를 `DELETE FROM`으로 비운 후 백업 데이터로 완벽하게 덮어씁니다.
- **원자적 트랜잭션 (Atomic ACID Guarantee)**:
  - 복원 중 정전이나 예외가 발생하더라도 SQLite `WAL` 모드 트랜잭션을 통해 `ROLLBACK`되므로 데이터베이스가 절대 손상되지 않습니다.
